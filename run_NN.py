...
liste_graphes_entrainement = [
    TrainingCycle(10, 3, 7, 39, 56),
    TrainingCycle(20, 18, 5, 85, 20),
    TrainingCycle(30, 22, 7, 6, 7),
]
liste_graphes_validation = [
    TrainingCycle(10, 5, 1, 84, 26),
    TrainingCycle(20, 11, 19, 25, 87),
    TrainingCycle(30, 13, 26, 57, 56)
]


def eval_genomes_entrainement(genomes, config):
    iterations_max = 1_000

    for genome_id, genome in genomes:
        genome.fitness = 0
        for graphe in liste_graphes_entrainement:
...
def eval_genomes_validation(genomes, config):
    iterations_max = 1_000
    fitnesses = {genome_id:0 for genome_id, genome in genomes}

    for genome_id, genome in genomes:
        for graphe in liste_graphes_validation:
            label1, label2 = graphe.label1, graphe.label2  # label: identifiant dans le graphe évalué
            taille_cycle = graphe.taille  # taille du graphe évalué
            ...
            if iterations <= iterations_max:
                fitnesses[genome_id] -= iterations
            else:
                fitnesses[genome_id] -= iterations_max * dist_min
    return fitnesses
...
def entrainement():
    ...
    p.add_reporter(neat_reporter.WANDB_Reporter(intervalle_de_validation=100, fonction_de_validation=eval_genomes_validation))
    ...
    run_result = p.run(eval_genomes_entrainement, 10_000 + 1)  # 10000 +1 pour être certain d'enregistrer le dernier checkpoint
